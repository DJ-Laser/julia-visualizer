...
fn hueShift(color: vec3f, u_theta: f32) -> vec3f {
  ...
  var yiqColor = mat3x3(0.299, 1.0, 0.40462981, 0.587, - 0.46081557, - 1.0, 0.114, - 0.53918443, 0.59537019) * color.rgb;
  yiqColor = vec3f(0, mat2x2(cos(u_theta), sin(u_theta), - sin(u_theta), cos(u_theta)) * yiqColor.yz);
  return mat3x3(1.0, 1.0, 1.0, 0.5696804, - 0.1620848, - 0.6590654, 0.3235513, - 0.3381869, 0.8901581) * yiqColor;
}
...
@vertex
fn vs_main(@location(0) vertex_uv: vec2f) -> VertexOutput {
  var out: VertexOutput;
  let x = vertex_uv.x * 2 - 1;
  let y = vertex_uv.y * 2 - 1;
